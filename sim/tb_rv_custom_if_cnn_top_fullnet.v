...
module tb_rv_custom_if_cnn_top_fullnet;
    ...
    wire if_acc_cmd_valid;
    wire [1:0] if_acc_cmd;
    wire [31:0] if_acc_desc_base;
    wire [31:0] if_acc_layer_num;
    wire acc_cmd_ready;
    ...
    rv_cnn_custom_if u_custom_if (
        ...
        .acc_cmd_valid(if_acc_cmd_valid),
        .acc_cmd(if_acc_cmd),
        .acc_desc_base(if_acc_desc_base),
        .acc_layer_num(if_acc_layer_num),
        .rd_data(rd_data),
        .rd_valid(rd_valid)
    );
    ...
    cnn_top u_cnn_top (
        .clk(clk),
        .rst_n(rst_n),
        .cmd_valid(if_acc_cmd_valid),
        .cmd_ready(acc_cmd_ready),
        .cmd_type(if_acc_cmd),
        .desc_base_addr(if_acc_desc_base),
        .layer_num(if_acc_layer_num),
        .status(acc_status),
    ...
    task issue_poll;
        ...
        begin
            ...
            @(posedge clk);
            #1;
            status_value = rd_valid ? rd_data : 32'hffff_ffff;
            instr_valid = 1'b0;
            @(posedge clk);
        end
    endtask
    ...
    task wait_done_by_poll;
        reg [31:0] polled_status;
        begin
            poll_count = 0;
            timeout = 0;
            polled_status = 32'd0;
            while (!polled_status[1] && timeout < 10000000) begin
                issue_poll(polled_status);
                timeout = timeout + 1;
                poll_count = poll_count + 1;
            end
            final_status = polled_status;
            if (timeout >= 10000000) begin
                $display("FAIL tb_rv_custom_if_cnn_top_fullnet timeout case=%0d status=%08x", case_idx, final_status);
                $fatal;
            end
        end
    endtask
    ...
    initial begin
        ...
        for (case_idx = 0; case_idx < num_cases; case_idx = case_idx + 1) begin
            ...
            if (scan_count != 4) begin
                $display("ERROR: bad rv custom fullnet header case=%0d scan=%0d", case_idx, scan_count);
                $fatal;
            end
            ...
            reset_dut();
            issue_start(desc_base_addr, layer_num_file[31:0]);
            wait_done_by_poll();
            issue_stat(stat_status);

            if (final_status[2]) begin
                $display("MISMATCH case=%0d rv custom IF reported error status=%08x", case_idx, final_status);
                error_count = error_count + 1;
            end
            if (stat_status !== {8'd0, final_status[31:8]}) begin
                $display("MISMATCH case=%0d stat_cycles=%08x poll_status=%08x", case_idx, stat_status, final_status);
                error_count = error_count + 1;
            end
            ...
            $display(
                "PERF tb_rv_custom_if_cnn_top_fullnet case=%0d polls=%0d status=%08x hw_cycles=%0d checks=%0d",
                case_idx,
                poll_count,
                final_status,
                final_status[31:8],
        ...
        if (error_count == 0 && total_checks > 0) begin
            $display("PASS tb_rv_custom_if_cnn_top_fullnet cases=%0d checks=%0d", num_cases, total_checks);
            $finish;
        end else begin
            $display("FAIL tb_rv_custom_if_cnn_top_fullnet checks=%0d errors=%0d", total_checks, error_count);
            $fatal;
        end
    end
endmodule
