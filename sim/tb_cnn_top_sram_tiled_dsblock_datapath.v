...
module tb_cnn_top_sram_tiled_dsblock_datapath;
    localparam MEM_WORDS = 131072;
    localparam MAX_EXPECTED = 8192;
        ...
        if (!rst_n) begin
            ...
            if (mem_req_valid && mem_req_ready) begin
                if (mem_req_write) begin
                    mem[mem_req_addr[18:2]] <= mem_req_wdata;
                end else begin
                    mem_resp_valid <= 1'b1;
                    mem_resp_rdata <= mem[mem_req_addr[18:2]];
                end
            end
        end
    end
    ...
    task start_and_wait_done;
        begin
            ...
            timeout = 0;
            while (!status[1] && timeout < 800000) begin
                timeout = timeout + 1;
                @(posedge clk);
            end
            if (timeout >= 800000) begin
                $display("FAIL tb_cnn_top_sram_tiled_dsblock_datapath timeout case=%0d status=%08x", case_idx, status);
                $fatal;
            end
        end
    endtask
    ...
    initial begin
        ...
        fd = $fopen("tests/vectors/cnn_top_sram_tiled_dsblock_cases.hex", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open tests/vectors/cnn_top_sram_tiled_dsblock_cases.hex");
            $finish;
        end
        ...
        for (case_idx = 0; case_idx < num_cases; case_idx = case_idx + 1) begin
            ...
            if (scan_count != 4) begin
                $display("ERROR: bad sram tiled dsblock header case=%0d scan=%0d", case_idx, scan_count);
                $fatal;
            end
            ...
            for (i = 0; i < expected_count; i = i + 1) begin
                if (mem[expected_addr[i][18:2]][7:0] !== expected_data[i]) begin
                    $display(
                        ...
                        $signed(mem[expected_addr[i][18:2]][7:0])
                    );
                    error_count = error_count + 1;
                end
        ...
        if (error_count == 0 && total_checks > 0) begin
            $display("PASS tb_cnn_top_sram_tiled_dsblock_datapath cases=%0d checks=%0d", num_cases, total_checks);
            $finish;
        end else begin
            $display("FAIL tb_cnn_top_sram_tiled_dsblock_datapath checks=%0d errors=%0d", total_checks, error_count);
            $fatal;
        end
    end
endmodule
